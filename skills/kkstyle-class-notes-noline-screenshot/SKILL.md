---
name: kkstyle-class-notes-noline-screenshot
description: "把文章、笔记、课程摘要或推荐清单生成成手写课堂笔记无描边、弱边界的信息海报 HTML，并可按固定比例截图为 PNG。Use when the user wants a no-line version, borderless version, softened-edge version, or a class notes poster, handwritten study board, marker-highlight layout, school notebook page, or both HTML and image output in a more note-like direction."
metadata:
  clawdbot:
    requires:
      bins:
...
# KKStyle Class Notes No-Line Screenshot

## Overview

这个 skill 用来把原始信息做成“课堂笔记、手写标记、纸张笔记页”风格，而不是海豹贴纸风、拼贴风、编辑设计风或像素掌机风。
默认优先产出 `3:4` 的 no-line class-notes poster：手写题目、荧光笔高亮、双栏笔记块、顶部日期标签，以及固定二维码 + IP 右下角组合。
