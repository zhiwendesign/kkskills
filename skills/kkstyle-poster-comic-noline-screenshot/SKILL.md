---
name: kkstyle-poster-comic-noline-screenshot
description: "把文章、笔记、课程摘要或推荐清单生成成漫画海报无描边、弱边界的信息海报 HTML，并可按固定比例截图为 PNG。Use when the user wants a no-line version, borderless version, softened-edge version, or a comic poster, pop-art cover, speech-bubble info card, bold cartoon-style layout, or both HTML and image output in a louder poster direction."
metadata:
  clawdbot:
    requires:
      bins:
...
# KKStyle Poster Comic No-Line Screenshot

## Overview

这个 skill 用来把原始信息做成“漫画海报、pop-art、高饱和块面、对话框”风格，而不是课堂笔记风、极简科技风或编辑设计风。
默认优先产出 `3:4` 的 no-line poster-comic poster：爆炸标题、对话框摘要、右上亮色爆点牌、双栏内容区、顶部日期贴纸，以及固定二维码 + IP 右下角组合。
...
- 背景用亮黄、亮红、亮蓝、奶白，辅以 halftone 点阵和柔和块面分区。
- 标题要像漫画封面字，粗、炸、直接。
- 摘要可以放在对话框或圆角气泡里。
