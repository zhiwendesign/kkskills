---
name: kkstyle-diagram-board-screenshot
description: "把文章、笔记、提纲或项目说明生成成信息图白板风格的信息海报 HTML，并可按固定比例截图为 PNG。Use when the user wants a diagram board, flowchart poster, whiteboard explainer, sticky-note infographic, or both HTML and image output in a more structured visual-explanation direction."
metadata:
  clawdbot:
    requires:
      bins:
...
默认优先产出 `3:4` 的 diagram-board poster：大标题、右上角数据卡、摘要条、多模块流程板、连接箭头感，以及固定日期 + 固定二维码 + 固定卡卡 IP。
