...
description: "把文章、笔记、课程摘要或推荐清单生成成复古像素游戏界面无描边、弱边界的信息海报 HTML，并可按固定比例截图为 PNG。Use when the user wants a no-line version, borderless version, softened-edge version, or a retro pixel poster, game UI board, arcade handheld style, chunky HUD-inspired info card, or both HTML and image output in a stronger pixel direction than the default kkstyle poster."
metadata:
  clawdbot:
    requires:
      bins:
...
# KKStyle Retro Pixel No-Line Screenshot
