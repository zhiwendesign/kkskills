...
This repository is a shareable Codex skill collection.

## Canonical source of truth

- Edit the top-level `kkstyle-*` directories.
- Treat `.Codex/skills/*` as Codex discovery links, not the primary place to edit.
...
- These skills are intentionally manual-first in Codex. They are invoked with `/kkstyle-...` instead of auto-triggering in unrelated sessions.
...
2. Refresh the Codex project links:

```bash
./scripts/install_claude_skills.sh --target ./.Codex/skills --mode link --force
```
